        ...
        with st.spinner("Breaking down shots & engineering creative prompts..."):
            try:
                ...
                for i, shot in enumerate(shots):
                    with cols[i]:
                        st.markdown(f"### Shot {i+1}: {shot.get('shot_type', '')}")
                        st.markdown(f"**Action:** {shot.get('action_description', '')}")
                        st.markdown(f"🎥 **Camera:** {shot.get('camera_movement', '')}")
                        st.markdown(f"🔍 **Lens:** {shot.get('lens_choice', '')}")
                        st.markdown(f"💡 **Lighting:** {shot.get('lighting', '')}")
                        
                        st.text_area(
                            f"Node Prompt (Shot {i+1})",
                            value=shot.get('visual_prompt', ''),
                            height=120,
                            key=f"prompt_{i}"
                        )
